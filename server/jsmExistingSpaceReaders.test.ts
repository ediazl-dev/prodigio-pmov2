...
import {
  inspectJsmExistingSpace,
  type InspectJsmExistingSpaceDependencies,
} from "./jsmExistingSpaceService";
...
describe("J2 JSM read-only readers", () => {
            ...
            values: [
              {
                id: "10",
                projectId: "10010",
                projectName: "Mesa Uno",
                projectKey: "MESA1",
              },
              {
                id: "11",
                projectId: "10011",
                projectName: "Mesa Dos",
                projectKey: "MESA2",
              },
            ],
          }),
  ...
  it("devuelve un catálogo vacío sin reintentos ni escrituras", async () => {
    ...
    const { listJsmServiceDesks } = await import("./jiraClient");
    const desks = await listJsmServiceDesks();
...
describe("J2 JSM preflight evaluator", () => {
  const makeDependencies = (
    overrides: Partial<InspectJsmExistingSpaceDependencies> = {}
  ): InspectJsmExistingSpaceDependencies => ({
    getServiceDesk: vi.fn(async () => ({
      id: "25",
      projectId: "10250",
  ...
  it("declara listo un Space JSM accesible y libre sin ejecutar escrituras", async () => {
    ...
    const result = await inspectJsmExistingSpace(
      { serviceDeskId: "25", serviceId: 2040001 },
      dependencies
    );
  ...
  it("bloquea un Service Desk que la cuenta técnica no puede consultar", async () => {
    const dependencies = makeDependencies({
      getPermissions: vi.fn(async () => ({
        BROWSE_PROJECTS: { key: "BROWSE_PROJECTS", havePermission: false },
        CREATE_ISSUES: { key: "CREATE_ISSUES", havePermission: true },
      })),
    });
    ...
    expect(result.status).toBe("service_desk_not_accessible");
    expect(result.canLink).toBe(false);
    expect(result.blockers).toContain(
      "La cuenta técnica no tiene permiso para consultar el proyecto Jira asociado."
    );
  });
});
