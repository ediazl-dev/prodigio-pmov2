...
import { createJiraSpace, createJiraIssue, CreateIssueInput, searchJiraIssues } from "./jiraClient";
import {
  listRecurringServices, getRecurringServiceById, createRecurringService, updateRecurringService,
  getRecurringServiceStages, getRecurringServiceStage, completeRecurringStage,
  getBillingMonths, saveBillingMonths, updateBillingMonthStatus, updateBillingMonthJiraKey,
  getServiceDocuments, insertServiceDocument, deleteServiceDocument,
  getWorkPlanItems, insertWorkPlanItem, updateWorkPlanItem, deleteWorkPlanItem, bulkInsertWorkPlanItems, updateWorkPlanItemJiraKey, deleteAllWorkPlanItems,
...
const adminOrPmo = protectedProcedure.use(({ ctx, next }) => {
  if (!["admin", "pmo"].includes(ctx.user.role))
    throw new TRPCError({ code: "FORBIDDEN", message: "Requiere rol Admin o PMO" });
  return next({ ctx });
});
...
export const recurringServicesRouter = router({
  ...
  dashboardKpis: protectedProcedure
    .query(async () => {
      ...
      // ── Status counts ──
      const statusCounts = { total: services.length, activo: 0, pausado: 0, completado: 0, cancelado: 0 };
      ...
      const monthlyBilling: { month: string; facturado: number; pagado: number; pendiente: number }[] = [];
      for (let i = 5; i >= 0; i--) {
        const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
        const monthBills = billingMonths.filter((b: any) => b.dueDate && b.dueDate.startsWith(key));
        let facturado = 0, pagado = 0, pendiente = 0;
      ...
      let currentMonthTotal = 0, currentMonthPaid = 0, currentMonthPending = 0;
      ...
      for (const b of billingMonths) {
        ...
        else if (b.status === "facturado") { /* facturado but not paid */ }
      ...
      return {
        ...
        billing: {
          currentMonth: { total: currentMonthTotal, paid: currentMonthPaid, pending: currentMonthPending },
  ...
  create: adminOrPmo
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await audit(ctx, "create", "recurring_service", id, input.serviceName, { clientName: input.clientName, serviceType: input.serviceType });
      return { id };
    }),
  ...
  updateStatus: adminOrPmo
    .input(z.object({ id: z.number(), status: z.enum(["activo", "pausado", "completado", "cancelado"]) }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.id);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
  ...
  saveBillingPlan: adminOrPmo
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      await saveBillingMonths(input.serviceId, input.months.map(m => ({
  ...
  deleteDocument: adminOrPmo
    .input(z.object({ serviceId: z.number(), documentId: z.number() }))
    .mutation(async ({ ctx, input }) => {
  ...
  syncPipedrive: adminOrPmo
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (isNaN(dealIdNum)) throw new TRPCError({ code: "BAD_REQUEST", message: "Deal ID debe ser un número válido" });
      ...
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: `Error al conectar con Pipedrive: ${msg}` });
      }
  ...
  generatePipedriveAiSummary: adminOrPmo
    .input(z.object({ serviceId: z.number() }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      if (!svc.dealId) throw new TRPCError({ code: "BAD_REQUEST", message: "No hay Deal ID de Pipedrive vinculado" });

      // Parse stored notes and flow summary for richer AI context
      let notesText = "Sin notas disponibles";
      if (svc.pipedriveNotesRaw) {
        try {
          const notes = JSON.parse(svc.pipedriveNotesRaw as string) as { content: string; date: string }[];
          notesText = notes.map((n, i) => `Nota ${i + 1} (${n.date}): ${n.content}`).join("\n");
        } catch { notesText = "Error al parsear notas"; }
      }
      ...
      if (svc.pipedriveFlowSummary) {
        try {
          ...
          const actTypes = Object.entries(flow.activityTypes || {}).map(([k, v]) => `${k}: ${v}`).join(", ");
        ...
        } catch { flowText = "Error al parsear flujo"; }
      }
...
Responde en español con formato JSON:
{
  ...
  "recommendations": "..."
      ...
      const response = await invokeLLM({
        messages: [
          { role: "system", content: "Eres un analista CRM senior de Prodigio Tech. Responde siempre en español." },
        ...
        response_format: {
          ...
          json_schema: {
            ...
            schema: {
              type: "object",
              properties: {
                summary: { type: "string", description: "Resumen ejecutivo del deal" },
                clientConcerns: { type: "string", description: "Preocupaciones del cliente" },
                recommendations: { type: "string", description: "Recomendaciones" },
              },
      ...
      if (!content) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "LLM no generó respuesta" });
  ...
  confirmInitStep1: adminOrPmo
    .input(z.object({ serviceId: z.number() }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      const stage = await getRecurringServiceStage(input.serviceId, "inicializacion");
      if (!stage || stage.status !== "in_progress") throw new TRPCError({ code: "BAD_REQUEST", message: "Etapa no está en progreso" });
      // Validate: billing plan must exist
      const billing = await getBillingMonths(input.serviceId);
      if (billing.length === 0) throw new TRPCError({ code: "BAD_REQUEST", message: "Debe configurar el plan de cobro antes de confirmar" });
      await updateRecurringService(input.serviceId, { initStep1Confirmed: true });
      await audit(ctx, "confirm_init_step1", "recurring_service", input.serviceId, svc.serviceName, { billingMonths: billing.length });
    }),

  closeInitializationStage: adminOrPmo
    .input(z.object({ serviceId: z.number() }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      ...
      if (!svc.initStep1Confirmed) throw new TRPCError({ code: "BAD_REQUEST", message: "Debe confirmar el Paso 1 (datos generales y plan de cobro) antes de cerrar" });
      // Validate documents: at least one document uploaded
      const docs = await getServiceDocuments(input.serviceId);
      if (docs.length === 0) throw new TRPCError({ code: "BAD_REQUEST", message: "Debe subir al menos un documento (propuesta, P&L, SoW o contrato)" });
      // Validate Pipedrive sync
      if (!svc.dealId) throw new TRPCError({ code: "BAD_REQUEST", message: "Debe sincronizar la información de Pipedrive antes de cerrar" });
  ...
  setFormalStartDate: adminOrPmo
    .input(z.object({ serviceId: z.number(), formalStartDate: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
...
Responde en JSON con este formato:
{
  ...
  "slaConfig": [
    { "priority": "critical|high|medium|low", "firstResponseMinutes": 30, "resolutionMinutes": 240, "coverageType": "24x7|8x5|personalizado" }
      ...
      const response = await invokeLLM({
        messages: [
          { role: "system", content: "Eres un Gerente de Proyectos Senior de Prodigio Tech. Genera planes de trabajo detallados para servicios recurrentes. Responde siempre en español." },
          { role: "user", content: prompt },
        ],
        response_format: {
          type: "json_schema",
      ...
      await audit(ctx, "ai_generate", "recurring_service", input.serviceId, svc.serviceName, { type: "work_plan", itemCount: items.length, slaCount: slaItems.length });
  ...
  saveSlaConfig: adminOrPmo
    ...
    .mutation(async ({ ctx, input }) => {
      await saveSlaConfig(input.serviceId, input.items.map(i => ({
        serviceId: input.serviceId,
        ...i,
      })));
  ...
  closeWorkPlanStage: adminOrPmo
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!stage || stage.status !== "in_progress") throw new TRPCError({ code: "BAD_REQUEST", message: "Etapa no está en progreso" });
      if (!svc.formalStartDate) throw new TRPCError({ code: "BAD_REQUEST", message: "Debe definir la fecha formal de inicio" });
      const items = await getWorkPlanItems(input.serviceId);
      if (items.length === 0) throw new TRPCError({ code: "BAD_REQUEST", message: "Debe tener al menos un ítem en el plan de trabajo" });
  ...
  createJsmProject: adminOrPmo
    ...
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      if (svc.jsmPlatform !== "prodigio") throw new TRPCError({ code: "BAD_REQUEST", message: "Solo se puede crear proyecto JSM cuando la plataforma es Prodigio" });
      ...
      if (result.success && result.created) {
        await updateRecurringService(input.serviceId, {
          ...
          jsmPortalUrl: result.jiraProjectUrl,
        });
        await audit(ctx, "jsm_project_create", "recurring_service", input.serviceId, svc.serviceName, {
          jiraKey: result.jiraProjectKey,
          jiraUrl: result.jiraProjectUrl,
        });
      }
      return result;
    }),

  syncJsmTasks: adminOrPmo
    .input(z.object({ serviceId: z.number() }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      if (!svc.jsmProjectKey) throw new TRPCError({ code: "BAD_REQUEST", message: "No hay proyecto JSM vinculado" });

      const workItems = await getWorkPlanItems(input.serviceId);
      const results: { id: number; title: string; status: string; jiraKey?: string; error?: string }[] = [];

      for (const item of workItems) {
        if (item.jiraIssueKey) {
          results.push({ id: item.id, title: item.title, status: "skipped", jiraKey: item.jiraIssueKey });
          continue;
        }
        try {
          const issueInput: CreateIssueInput = {
            ...
            description: item.description ?? "",
            issueTypeName: "Task",
            labels: [item.itemType, `mes-${item.monthNumber ?? 0}`],
          };
          if (item.dueDate) issueInput.dueDate = item.dueDate;
          const created = await createJiraIssue(issueInput);
          await updateWorkPlanItemJiraKey(item.id, created.key);
          results.push({ id: item.id, title: item.title, status: "created", jiraKey: created.key });
        } catch (e: any) {
          results.push({ id: item.id, title: item.title, status: "error", error: e.message?.substring(0, 200) });
        }
      }

      // Also create billing milestones as issues
      const billing = await getBillingMonths(input.serviceId);
      for (const month of billing) {
        if (month.jiraIssueKey) continue;
        try {
          const created = await createJiraIssue({
            projectKey: svc.jsmProjectKey!,
            summary: `Facturación Mes ${month.monthNumber} - ${svc.serviceName}`,
            description: `Cobro mensual #${month.monthNumber}: ${month.amount} ${month.currency}`,
            issueTypeName: "Task",
            labels: ["facturacion", `mes-${month.monthNumber}`],
            dueDate: month.dueDate ? ((month.dueDate as any) instanceof Date ? (month.dueDate as any).toISOString().split("T")[0] : String(month.dueDate)) : undefined,
          });
          await updateBillingMonthJiraKey(month.id, created.key);
        } catch (e: any) {
          // Non-blocking
        }
      }

      await audit(ctx, "jsm_issues_sync", "recurring_service", input.serviceId, svc.serviceName, {
  ...
  getJiraIssuesSummary: protectedProcedure
    .input(z.object({ serviceId: z.number() }))
    .query(async ({ input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      ...
      const syncedWorkItems = workItems.filter(w => w.jiraIssueKey).map(w => ({
        id: w.id, jiraKey: w.jiraIssueKey!, title: w.title, type: w.itemType,
        frequency: w.frequency, monthNumber: w.monthNumber, category: "actividad" as const,
      }));
      const unsyncedWorkItems = workItems.filter(w => !w.jiraIssueKey).map(w => ({
        id: w.id, title: w.title, type: w.itemType,
        frequency: w.frequency, monthNumber: w.monthNumber, category: "actividad" as const,
      }));
      const syncedBilling = billing.filter(b => b.jiraIssueKey).map(b => ({
        id: b.id, jiraKey: b.jiraIssueKey!, title: `Facturación Mes ${b.monthNumber}`,
        amount: b.amount, currency: b.currency, monthNumber: b.monthNumber, category: "facturacion" as const,
      }));
      const unsyncedBilling = billing.filter(b => !b.jiraIssueKey).map(b => ({
        id: b.id, title: `Facturación Mes ${b.monthNumber}`,
        amount: b.amount, currency: b.currency, monthNumber: b.monthNumber, category: "facturacion" as const,
      }));
  ...
  resyncJsmTasks: adminOrPmo
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const workItems = await getWorkPlanItems(input.serviceId);
      const billing = await getBillingMonths(input.serviceId);
      const results: { id: number; title: string; status: string; jiraKey?: string; error?: string; category: string }[] = [];

      // Sync missing work items
      for (const item of workItems) {
        if (item.jiraIssueKey) {
          results.push({ id: item.id, title: item.title, status: "already_synced", jiraKey: item.jiraIssueKey, category: "actividad" });
          continue;
        }
        try {
          const issueInput: CreateIssueInput = {
          ...
          const created = await createJiraIssue(issueInput);
          await updateWorkPlanItemJiraKey(item.id, created.key);
          results.push({ id: item.id, title: item.title, status: "created", jiraKey: created.key, category: "actividad" });
        } catch (e: any) {
          results.push({ id: item.id, title: item.title, status: "error", error: e.message?.substring(0, 200), category: "actividad" });
        }
      }

      // Sync missing billing
      for (const month of billing) {
        if (month.jiraIssueKey) {
          results.push({ id: month.id, title: `Facturación Mes ${month.monthNumber}`, status: "already_synced", jiraKey: month.jiraIssueKey, category: "facturacion" });
          continue;
        }
        try {
          const created = await createJiraIssue({
          ...
          await updateBillingMonthJiraKey(month.id, created.key);
          results.push({ id: month.id, title: `Facturación Mes ${month.monthNumber}`, status: "created", jiraKey: created.key, category: "facturacion" });
        } catch (e: any) {
          results.push({ id: month.id, title: `Facturación Mes ${month.monthNumber}`, status: "error", error: e.message?.substring(0, 200), category: "facturacion" });
        }
      }

      const newCreated = results.filter(r => r.status === "created").length;
      if (newCreated > 0) {
        await audit(ctx, "jsm_issues_resync", "recurring_service", input.serviceId, svc.serviceName, {
          total: results.length,
          newCreated,
  ...
  closeJiraSetupStage: adminOrPmo
    .input(z.object({ serviceId: z.number() }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      ...
      if (!stage || stage.status !== "in_progress") throw new TRPCError({ code: "BAD_REQUEST", message: "Etapa no está en progreso" });
      // Must have either JSM project or client platform URL
      if (!svc.jsmProjectKey && !svc.jsmClientPlatformUrl) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Debe configurar la plataforma de gestión (JSM propio o URL del cliente)" });
      }
      // Validate that issues have been synced (if using Prodigio JSM)
      if (svc.jsmProjectKey) {
        const workItems = await getWorkPlanItems(input.serviceId);
        const syncedCount = workItems.filter(w => w.jiraIssueKey).length;
        if (syncedCount === 0) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Debe sincronizar las tareas al proyecto JIRA antes de cerrar esta etapa. Use el botón 'Sincronizar Tareas' primero." });
        }
      }
  ...
  getExecutionDashboard: protectedProcedure
    .input(z.object({ serviceId: z.number() }))
    .query(async ({ input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      const [billing, workItems, sla, penalties, latestAnalysis, analysisHistory] = await Promise.all([
        getBillingMonths(input.serviceId),
        getWorkPlanItems(input.serviceId),
        getSlaConfig(input.serviceId),
        getPenalties(input.serviceId),
        getLatestAiAnalysis(input.serviceId),
        getAiAnalysisHistory(input.serviceId, 5),
      ]);
      ...
      if (svc.jsmProjectKey) {
        try {
          const jiraResult = await searchJiraIssues(
            `project=${svc.jsmProjectKey} ORDER BY created ASC`,
            { maxResults: 200, fields: ["summary", "status", "duedate"] }
          );
        ...
        } catch { /* JIRA unavailable - continue without */ }
      }
      ...
      if (latestAnalysis) {
        try { aiAnalysis = JSON.parse(latestAnalysis.fullAnalysis ?? "{}"); } catch { /* ignore */ }
        ...
        aiDimensions = {
          sla: { status: latestAnalysis.slaDimension, score: latestAnalysis.slaScore, detail: latestAnalysis.slaDetail },
          deliverables: { status: latestAnalysis.deliverablesDimension, score: latestAnalysis.deliverablesScore, detail: latestAnalysis.deliverablesDetail },
          billing: { status: latestAnalysis.billingDimension, score: latestAnalysis.billingScore, detail: latestAnalysis.billingDetail },
        };
      } else if (svc.aiFullAnalysis) {
        try { aiAnalysis = JSON.parse(svc.aiFullAnalysis); } catch { /* ignore */ }
      ...
      return {
        ...
        metrics: {
          ...
          slaFactors: {
            ...
            monthsElapsed,
            reason: totalItems === 0 && jiraTotalIssues === 0
              ? "no_tickets"
              : overdueTasks.length > 0 || jiraOverdueCount > 0
                ? "overdue_items"
                : completedItems === 0 && jiraCompletedIssues === 0 && monthsElapsed > 0
                  ? "zero_progress"
                  : hasSlaConfig
                    ? "sla_penalty_calc"
                    : "progress_based",
  ...
  getWorkPlanFromJira: protectedProcedure
    .input(z.object({ serviceId: z.number() }))
    .query(async ({ input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      ...
      try {
        const jiraResult = await searchJiraIssues(
          `project=${svc.jsmProjectKey} ORDER BY created ASC`,
          { maxResults: 200, fields: ["summary", "status", "issuetype", "assignee", "priority", "created", "updated", "duedate", "labels"] }
        );
        for (const issue of jiraResult.issues) {
          jiraIssuesMap.set(issue.key, issue);
        }
      } catch (e: any) {
        return { workItems: [], billingItems: [], jiraUnavailable: true, error: e.message?.substring(0, 200) };
      }
  ...
  getOrRefreshAiAnalysis: protectedProcedure
    .input(z.object({ serviceId: z.number(), forceRefresh: z.boolean().default(false) }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
          ...
          if (daysDiff <= 5) {
            try {
              ...
              return {
                ...
                daysOld: daysDiff,
                dimensions: {
                  sla: { status: latest.slaDimension, score: latest.slaScore, detail: latest.slaDetail },
                  deliverables: { status: latest.deliverablesDimension, score: latest.deliverablesScore, detail: latest.deliverablesDetail },
                  billing: { status: latest.billingDimension, score: latest.billingScore, detail: latest.billingDetail },
                },
              };
            } catch { /* regenerate */ }
          }
...
Responde en JSON.`;
      ...
      const response = await invokeLLM({
        messages: [
          { role: "system", content: "Eres un Gerente de Proyectos Senior de Prodigio Tech. Analiza servicios recurrentes con perspectiva ejecutiva. Responde en español." },
          { role: "user", content: prompt },
        ],
        response_format: {
          type: "json_schema",
          json_schema: {
            ...
            schema: {
              type: "object",
              properties: {
                semaphore: { type: "string", description: "VERDE, AMARILLO o ROJO" },
                semaphoreJustification: { type: "string" },
                executiveAbstract: { type: "string", description: "1 párrafo resumen ejecutivo corto" },
                executiveSummary: { type: "string", description: "Análisis detallado 2-3 párrafos" },
                dimensions: {
                  type: "object",
                  properties: {
                    sla: {
                      type: "object",
                      properties: {
                        status: { type: "string", description: "VERDE, AMARILLO o ROJO" },
      ...
      await audit(ctx, "ai_generate", "recurring_service", input.serviceId, svc.serviceName, {
        ...
        semaphore: parsed.semaphore,
        dimensions: { sla: parsed.dimensions?.sla?.status, deliverables: parsed.dimensions?.deliverables?.status, billing: parsed.dimensions?.billing?.status },
      });
  ...
  updatePenaltyStatus: adminOrPmo
    .input(z.object({ id: z.number(), serviceId: z.number(), status: z.enum(["identificada", "aplicada", "disputada", "resuelta"]) }))
    .mutation(async ({ ctx, input }) => {
  ...
  generateAiExecutiveSummary: adminOrPmo
    .input(z.object({ serviceId: z.number() }))
    .mutation(async ({ ctx, input }) => {
      ...
      const response = await invokeLLM({
        messages: [
          { role: "system", content: "Eres un Gerente de Proyectos Senior de Prodigio Tech. Analiza servicios recurrentes. Responde en español." },
          { role: "user", content: prompt },
        ],
        response_format: {
          type: "json_schema",
            ...
            schema: {
              ...
              properties: {
                ...
                risks: {
                  ...
                  items: {
                    type: "object",
                    properties: { level: { type: "string" }, description: { type: "string" }, mitigation: { type: "string" } },
                    required: ["level", "description", "mitigation"],
                    additionalProperties: false,
                  },
                },
                recommendations: {
                  ...
                  items: {
                    type: "object",
                    properties: { priority: { type: "integer" }, title: { type: "string" }, description: { type: "string" } },
                    required: ["priority", "title", "description"],
                    additionalProperties: false,
                  },
                },
  ...
  closeService: adminOrPmo
    .input(z.object({ serviceId: z.number(), notes: z.string().optional() }))
    .mutation(async ({ ctx, input }) => {
      const svc = await getRecurringServiceById(input.serviceId);
      if (!svc) throw new TRPCError({ code: "NOT_FOUND" });
      const stage = await getRecurringServiceStage(input.serviceId, "cierre");
      if (!stage || stage.status !== "in_progress") throw new TRPCError({ code: "BAD_REQUEST", message: "Etapa de cierre no está en progreso" });
