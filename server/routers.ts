  ...
  generatePMAnalysis: protectedProcedure.input(z.object({ projectId: z.number() }))
    .mutation(async ({ input, ctx }) => {
      ...
      const jiraSummary = `DATOS JIRA — SNAPSHOT LOCAL ${projectKey}\n` +
        ...
        `Pipeline PMO separado: ${portfolioEvidence?.stageLabel ?? "N/D"} (${portfolioEvidence?.stagesClosed ?? 0}/${portfolioEvidence?.totalStages ?? 6} etapas completadas)\n` +
...
const financialRouter = router({
  ...
  syncLogs: adminOnly.input(z.object({
    page: z.number().int().min(1).default(1),
    pageSize: z.literal(20).default(20),
  }).optional()).query(async ({ input }) => {
    return getFinancialSyncLogPage(input?.page ?? 1, input?.pageSize ?? 20);
  }),
