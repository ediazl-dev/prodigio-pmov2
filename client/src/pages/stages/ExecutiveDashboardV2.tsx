...
export type ExecutiveDashboardV2Props = {
  projectIdOverride?: number;
  embedded?: boolean;
};

export default function ExecutiveDashboardV2({ projectIdOverride, embedded = false }: ExecutiveDashboardV2Props = {}) {
  const { user, loading: authLoading } = useAuth({ redirectOnUnauthenticated: true });
  const params = useParams<{ id: string }>();
  const projectId = projectIdOverride ?? Number(params.id);
  ...
  const embeddedStateStyle: React.CSSProperties | undefined = embedded
    ? { minHeight: 180, display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid #DDE3EA", borderRadius: 14, background: "#FFFFFF", color: "#5A6472", padding: 24 }
    : undefined;
  if (authLoading || dashboard.isLoading) return <div className="edv2-load" data-dashboard-presentation={embedded ? "embedded" : "standalone"} style={embeddedStateStyle}><Loader2 className="h-5 w-5 animate-spin" />Cargando Dashboard Ejecutivo v2…</div>;
  if (dashboard.error || !dashboard.data) return <div className="edv2-load" data-dashboard-presentation={embedded ? "embedded" : "standalone"} style={embeddedStateStyle}><div><b>Dashboard Ejecutivo v2 pendiente de habilitación.</b><p>{dashboard.error?.message ?? "No existe un baseline ejecutivo aprobado para este proyecto."}</p>{embedded ? null : <a href={`/projects/${projectId}`}><ArrowLeft size={15} />Volver al proyecto</a>}</div></div>;
  ...
  return <main className={`edv2-root${embedded ? " edv2-root--embedded" : ""}`} data-dashboard-presentation={embedded ? "embedded" : "standalone"}>
    <style>{`
    ...
    <style>{`
      ...
      .edv2-root--embedded{width:100%;max-width:100%;min-height:0;overflow-x:clip;border:1px solid var(--line);border-radius:14px;box-shadow:0 2px 16px rgba(10,22,40,.08)}
      .edv2-root--embedded .edv2-hero{border-radius:14px 14px 0 0}
      .edv2-root--embedded .edv2-side-nav{top:0;min-height:auto}
      .edv2-root--embedded .edv2-wrap,.edv2-root--embedded .edv2-main-content{max-width:100%;min-width:0}
      .edv2-root--embedded .edv2-section{scroll-margin-top:16px}
    ...
    <div className="edv2-body-shell">
      <aside className="edv2-side-nav" aria-label="Navegación lateral del dashboard ejecutivo">
        ...
        {!embedded ? <a className="edv2-side-nav-return" href={`/projects/${projectId}`}>← Volver al proyecto</a> : null}
      </aside>
...
<tr><td>Capacity / stop-loss</td><td>[POR CONFIRMAR]</td><td>Sincronización financiera pendiente</td><td>No proyecta carry ni autoriza continuidad sin insumos aprobados.</td></tr>
    ...
    {!embedded ? <footer className="edv2-footer"><div className="edv2-wrap"><span><b>Prodigio · Dashboard Ejecutivo v2</b> · evidencia contractual primero</span><span>SoW {source.baselineVersion} · Deal {source.dealId} · Jira {source.jiraProjectKey}</span></div></footer> : null}
  </main>;
}
