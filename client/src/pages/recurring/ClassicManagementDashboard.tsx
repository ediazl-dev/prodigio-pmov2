...
const RECONCILIATION_LABELS: Record<string, { label: string; className: string }> = {
  ...
  missing_invoice: { label: "Sin registro corporativo", className: "border-amber-200 bg-amber-50 text-amber-900" },
...
function BillingEvolution({
  ...
  if (visible.length === 0) {
    return <p className="rounded-xl border border-dashed border-slate-300 p-5 text-sm text-slate-600">No hay programación ni registros corporativos {currency} en la ventana seleccionada.</p>;
  }
  const max = Math.max(...visible.flatMap(row => [row.expected, row.invoiced, row.future]), 1);
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3" aria-label={`Esperado versus facturación registrada en ${currency}`}>
      {visible.map(row => (
        <article key={`${row.month}:${row.currency}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="mt-3 space-y-2 text-[10px]">
            {[
              ...
              ["Registro corporativo", row.invoiced, "bg-blue-600"],
...
function FinanceEvidencePanel({ data }: { data: RouterOutputs["recurringServices"]["dashboardV2"] }) {
  return (
    <div className="p-5 sm:p-6">
      <div className="flex items-center gap-2"><ReceiptText size={16} className="text-[#175CD3]" /><h4 className="text-sm font-black text-slate-950">Programación contractual y fuente financiera corporativa</h4></div>
      <p className="mt-1 text-[11px] text-slate-600">La facturación se acredita sólo cuando la fuente financiera contiene un registro marcado Facturado; esto no sustituye una factura SII.</p>
        ...
        {data.management.services.map(service => {
          ...
          return (
            <article key={service.serviceId} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-[10px]"><div><p className="font-bold uppercase text-slate-500">Plan esperado</p><p className="mt-1 font-mono font-black text-slate-950">{expected.join(" · ") || "N/D"}</p></div><div><p className="font-bold uppercase text-slate-500">Fuente corporativa</p><p className="mt-1 font-mono font-black text-emerald-700">{invoiced.join(" · ") || "Sin registro facturado"}</p></div></div>
  ...
  return (
    <div className="mb-8 space-y-5" data-testid="classic-management-dashboard">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-600">Separa programación contractual, fuente financiera corporativa, operación JSM y evidencia de gobierno. El ciclo financiero termina en <b>Facturado</b>.</p>
          </div>
      ...
      <section aria-label="Respuestas gerenciales inmediatas" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        ...
        <MetricCard label="Fuente financiera" value={`${model.summary.withVerifiedInvoices}/${model.summary.services}`} note="Con al menos un registro corporativo marcado Facturado." icon={ReceiptText} tone="green" />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        {activeCurrency ? <>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <MetricCard label="Facturación registrada" value={formatMoney(activeCurrency.invoicedReal, activeCurrency.currency)} note={`${activeCurrency.invoiceContributors.reduce((sum, item) => sum + item.invoices, 0)} registro(s) corporativo(s) marcado(s) Facturado.`} icon={ReceiptText} tone="green" />
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            ...
            <article className="rounded-xl border border-slate-200 bg-slate-50 p-4"><h4 className="text-xs font-black text-slate-900">Registros facturados atribuidos</h4><div className="mt-3 space-y-2">{activeCurrency.invoiceContributors.length === 0 ? <p className="text-xs font-bold text-amber-800">No hay facturación {activeCurrency.currency} registrada en la fuente corporativa.</p> : activeCurrency.invoiceContributors.map(item => <div key={item.serviceId} className="flex items-center justify-between gap-3 rounded-lg bg-white px-3 py-2 text-[11px]"><span><b className="block text-slate-900">{item.clientName}</b><span className="text-slate-500">{item.invoices} registro(s) · {item.serviceName}</span></span><b className="font-mono text-emerald-700">{formatMoney(item.amount, activeCurrency.currency)}</b></div>)}</div></article>
          </div>
          ...
          <div className="mt-5 border-t border-slate-200 pt-5"><div className="mb-3"><h4 className="text-sm font-black text-slate-950">Esperado vs. facturación registrada</h4><p className="mt-1 text-[11px] text-slate-600">Lectura {effectiveControls.comparison === "monthly" ? "mensual" : "acumulada"}; las cuotas futuras usan una señal visual separada.</p></div><BillingEvolution rows={model.financeMonthly} currency={activeCurrency.currency} comparison={effectiveControls.comparison} /></div>
